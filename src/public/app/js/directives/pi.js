        ...
        return function postLink(scope, el, attrs, ctl) {
          ...
          el.bind('change', function (e) {
            ...
            if (e.target.files.length > scope.maxFiles) {
              raiseError(
                e.target.files,
                'TOO_MANY_FILES',
                "Cannot upload " + e.target.files.length + " files, maxium allowed is " + scope.maxFiles
              );
              return;
            }
            
            for (let i = 0; i < scope.maxFiles; i++) {
          ...
          scope.upload = function () {
            ...
            fileUploader
              ...
              .to(uploadPath)
              .then(function (ret) {
                scope.ondone({files: objFiles, data: ret.data, status: ret.status});
              }, function (error) {
